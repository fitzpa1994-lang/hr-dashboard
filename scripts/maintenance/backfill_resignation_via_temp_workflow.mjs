...
const webhookPath = `tmp-resignation-backfill-${crypto.randomBytes(8).toString('hex')}`;
...
SELECT
  ...
  DATE '${escapeSql(lastDay)}' AS last_day,
...
function buildWorkflowPayload() {
  return {
    name: `tmp-resignation-backfill-${name}`,
    nodes: [
      {
        id: 'webhook-trigger',
        name: 'Webhook',
...
async function main() {
  ...
  try {
    ...
    const executeResponse = await fetch(webhookUrl, {
      ...
      body: JSON.stringify({ source: 'codex-resignation-backfill' }),
    });
